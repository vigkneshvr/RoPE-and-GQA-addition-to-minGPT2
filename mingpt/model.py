...
class GroupedQueryAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.k_attn= nn.Linear(config.n_embd, config.n_embd//self.g)
        self.v_attn= nn.Linear(config.n_embd, config.n_embd//self.g)
        self.q_attn= nn.Linear(config.n_embd, config.n_embd)

        # output projection
        self.c_proj = nn.Linear(config.n_embd//self.g, config.n_embd)
    ...
    def forward(self, x):
        ...
        q= rearrange(q,"B T (nh d) -> B nh T d",nh=self.q_head)
        k= rearrange(k,"B T (nh d) -> B nh T d",nh=self.kv_head)
        v= rearrange(v,"B T (nh d) -> B nh T d", nh= self.kv_head)        

        q = rearrange(q, "b (h g) n d -> b g h n d", g=self.g)

        # causal self-attention; Self-attend: (B, nh, T, hs) x (B, nh, hs, T) -> (B, nh, T, T)
        torch.cuda.empty_cache()
        start_memory = torch.cuda.memory_allocated()
        att=einsum(q,k, "b g h n d, b h s d -> b h n s")* (1.0 / math.sqrt(k.size(-1)))
        att = att.masked_fill(self.bias[:,:,:T,:T] == 0, float('-inf'))
        att = F.softmax(att, dim=-1)
        att = self.attn_dropout(att)
        y = att @ v # (B, nh, T, T) x (B, nh, T, d) -> (B, nh, T, d)
